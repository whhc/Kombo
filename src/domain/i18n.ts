...
const UI_ZH: Dict = {
  ...
  'help.combo.optimalPath': '编辑器与列表卡片显示"最优键序":基于 FIFO 切球队列求解的最少按键序列,遵循"切一个合一个放一个"的实战节奏。',
  'help.combo.toggle': '练习页标题旁的 👁 按钮可切换最优键序的显示/隐藏,设置持久化。',
  ...
  'help.metrics.successRate': '成功率 = 成功轮次 / 总轮次(含失败)。徽章颜色:绿 ≥ 80%、黄 50-80%、红 < 50%。',
...
const UI_EN: Dict = {
  ...
  'help.combo.optimalPath': 'The editor and combo cards show an "optimal path": the fewest-keystroke sequence solved from the FIFO orb queue, following the "switch-invoke-cast one at a time" rhythm.',
  'help.combo.toggle': 'The 👁 button next to the practice title toggles optimal-path visibility; the setting persists.',
  ...
  'help.metrics.successRate': 'Success rate = successful rounds / total rounds (including failures). Badge color: green ≥ 80%, amber 50-80%, red < 50%.',
