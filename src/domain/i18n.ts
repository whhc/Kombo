...
const UI_ZH: Dict = {
  ...
  'help.practice.recipe': '自由练习页标题旁的 👁 眼睛按钮可切换显示"技能配方"参考面板:列出全部 10 个技能及其 3 元素配方(元素用图标,按 Q→W→E 顺序排列),方便不熟悉技能的用户随时查阅。注:此按钮与连招练习页的"最优键序"眼睛按钮是不同功能。',
...
const UI_EN: Dict = {
  ...
  'help.practice.recipe': 'The 👁 eye button next to the free-practice title toggles a "Spell Recipes" reference panel: lists all 10 spells with their 3-element recipes (elements shown as icons, ordered Q→W→E) for quick lookup by users unfamiliar with the spells. Note: this button is different from the "optimal path" eye button on the combo practice page.',
